...
def GHOST(impath, checkDisplacements=0):
    ...
    """
    imorig = cp.double(cv2.imread(impath))
    minQ = 51
    maxQ = 100
    stepQ = 1
    dispImages = {}
    Output = cp.zeros(int((maxQ-minQ)/stepQ+1))
    Mins = cp.zeros(int((maxQ-minQ)/stepQ+1))
    ...
    smoothing_b = 17
    h = (cp.ones((smoothing_b, smoothing_b))/(smoothing_b**2))[:, :, None]
    ...
    for ii in range(minQ, maxQ+1, stepQ):
        ...
        cparr = cp.frombuffer(encimg, cp.byte)
        tmpResave = cp.double(cv2.imdecode(cparr, cv2.IMREAD_ANYCOLOR))
        Deltas = {}
        overallDelta = {}
        ComparisonFull = (tmpResave-imorig)**2
        ComparisonFull = cp.double(fftconvolve(ComparisonFull, h, 'same'))
        for dispx in range(maxDisp+1):
            for dispy in range(maxDisp+1):
                ...
                Deltas[DisplacementIndex] = cp.mean(Comparison, axis=2)
                overallDelta[DisplacementIndex] = cp.mean(Deltas[DisplacementIndex])
        ...
        dispImages[int((ii-minQ)/stepQ)] = resize(cp.float32(delta), newSize)
    OutputX = range(minQ, maxQ+1, stepQ)
    OutputY = Output
    imin = signal.argrelextrema(OutputY, cp.less)[0]+1  # Add one to acct for matlab starting idx counting at 1.
    if(OutputY[-1] < OutputY[-2]):  # Check last point
        imin = cp.append(imin, len(OutputY))
    if(OutputY[0] < OutputY[1]):  # Check first point
        imin = cp.insert(imin, 0, 1)
